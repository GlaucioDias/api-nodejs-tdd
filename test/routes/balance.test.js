...
const TOKEN_GERAL = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MTAxMDIsIm5hbWUiOiJVc2VyICM1IiwibWFpbCI6InVzZXI1QHRlc3QuY29tIn0.I_gxvqZFvN7M_g2PAH9ntJMRpBohPkiJjS5V6bPELh8';
...
describe('Ao calcular o saldo do usuário...', () => {
  ...
  test('Deve adicionar valores de entrada', () => {
    return request(app).post(TRANSACTION_ROUTE)
      .set('authorization', `bearer ${TOKEN}`)
      .send({
        description: '1', date: new Date(), amount: 100, type: 'I', acc_id: 10100, status: true,
      })
      .then(() => {
        return request(app).get(MAIN_ROUTE)
          .set('authorization', `bearer ${TOKEN}`)
  ...
  test('Deve subtrair valores de saída', () => {
    return request(app).post(TRANSACTION_ROUTE)
      .set('authorization', `bearer ${TOKEN}`)
      .send({
        description: '1', date: new Date(), amount: 200, type: 'O', acc_id: 10100, status: true,
      })
      .then(() => {
        return request(app).get(MAIN_ROUTE)
          .set('authorization', `bearer ${TOKEN}`)
  ...
  test('Nâo deve considerar transações pendentes', () => {
    return request(app).post(TRANSACTION_ROUTE)
      .set('authorization', `bearer ${TOKEN}`)
      .send({
        description: '1', date: new Date(), amount: 100, type: 'O', acc_id: 10100, status: false,
      })
      .then(() => {
        return request(app).get(MAIN_ROUTE)
          .set('authorization', `bearer ${TOKEN}`)
  ...
  test('Nâo deve considerar saldo de contas distintas', () => {
    return request(app).post(TRANSACTION_ROUTE)
      .set('authorization', `bearer ${TOKEN}`)
      .send({
        description: '1', date: new Date(), amount: 50, type: 'I', acc_id: 10101, status: true,
      })
      .then(() => {
        return request(app).get(MAIN_ROUTE)
          .set('authorization', `bearer ${TOKEN}`)
  ...
  test('Nâo deve considerar contas de outros usuários', () => {
    return request(app).post(TRANSACTION_ROUTE)
      .set('authorization', `bearer ${TOKEN}`)
      .send({
        description: '1', date: new Date(), amount: 100, type: 'O', acc_id: 10102, status: true,
      })
      .then(() => {
        return request(app).get(MAIN_ROUTE)
          .set('authorization', `bearer ${TOKEN}`)
  ...
  test('Deve considerar uma transação passada', () => {
    return request(app).post(TRANSACTION_ROUTE)
      .set('authorization', `bearer ${TOKEN}`)
      .send({
        description: '1', date: moment().subtract({ days: 5 }), amount: 250, type: 'I', acc_id: 10100, status: true,
      })
      .then(() => {
        return request(app).get(MAIN_ROUTE)
          .set('authorization', `bearer ${TOKEN}`)
  ...
  test('Não deve considerar uma transação futura', () => {
    return request(app).post(TRANSACTION_ROUTE)
      .set('authorization', `bearer ${TOKEN}`)
      .send({
        description: '1', date: moment().add({ days: 5 }), amount: 250, type: 'I', acc_id: 10100, status: true,
      })
      .then(() => {
        return request(app).get(MAIN_ROUTE)
          .set('authorization', `bearer ${TOKEN}`)
  ...
  test('Deve considerar transferências', () => {
    return request(app).post(TRANSFER_ROUTE)
      .set('authorization', `bearer ${TOKEN}`)
      .send({
        description: '1', date: moment(), amount: 250, acc_ori_id: 10100, acc_dest_id: 10101,
      })
      .then(() => {
        return request(app).get(MAIN_ROUTE)
          .set('authorization', `bearer ${TOKEN}`)
          .then((res) => {
            ...
            expect(res.body[0].sum).toBe('-100.00');
            expect(res.body[1].id).toBe(10101);
            expect(res.body[1].sum).toBe('300.00'); // 300.00
          });
      });
  });
});
...
test('Deve calcular saldo das contas do usuário', () => {
  return request(app).get(MAIN_ROUTE)
    ...
    .then((res) => {
      ...
      expect(res.body[1].sum).toBe('-248.00'); // 300.00
    });
});
