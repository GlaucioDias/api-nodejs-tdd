...
module.exports = (app) => {
  ...
  const validate = async (transfer) => {
    ...
    if (transfer.acc_ori_id === transfer.acc_dest_id) throw new ValidationError('Conta de origem e destino não podem ser iguais');
  ...
  return {
    find, findOne, save, update, remove, validate,
  };
};
